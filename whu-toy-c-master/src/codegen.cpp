...
/* quick tree query: does expr contain a function call? */
static bool expr_contains_call(const Expr* e) {
    if (!e) return false;
    if (e->type == Expr::FUNC_CALL) return true;
    switch (e->type) {
        case Expr::UNARY_OP:
            return expr_contains_call(e->child);
        case Expr::BINARY_OP:
            return expr_contains_call(e->left) || expr_contains_call(e->right);
        default:
            return false;
    }
}
...
static void emit_call(Expr* call_expr, const FuncInfo& fi, ostream &o, int &cur_sp_bytes, bool push_return) {
    ...
    // fast path: no nested calls inside args and <= 8 args -> evaluate directly into a-registers
    bool any_call = false;
    for (auto *arg : call_expr->args) {
        if (expr_contains_call(arg)) { any_call = true; break; }
    }
    if (!any_call && argc <= 8) {
        // evaluate right-to-left to preserve previous evaluation order
        for (int i = (int)argc - 1; i >= 0; --i) {
            gen_expr_to_reg(call_expr->args[i], fi, o, cur_sp_bytes, string("a") + to_string(i));
        }
        // call with a0..a7 already set; ensure stack alignment (cur_sp_bytes should be 0 here)
        int pad = (16 - (cur_sp_bytes % 16)) % 16;
        if (pad > 0) {
    ...
    // fallback: original push-and-load strategy (right-to-left evaluation)
    size_t total_args_bytes = argc * 4;
    int pad = (16 - ((cur_sp_bytes + (int)total_args_bytes) % 16)) % 16;
    if (pad > 0) {
        emit(o, "addi sp, sp, -" + to_string(pad));
        cur_sp_bytes += pad;
    }
    ...
    if ((int)total_args_bytes + pad > 0) {
        emit(o, "addi sp, sp, " + to_string((int)total_args_bytes + pad));
        cur_sp_bytes -= ((int)total_args_bytes + pad);
    }
    if (push_return) {
        emit(o, "mv t0, a0");
        push_reg_t0(o, cur_sp_bytes);
...
static void gen_expr_stack(Expr* e, const FuncInfo& fi, ostream &o, int &cur_sp_bytes) {
    ...
    switch (e->type) {
        ...
        case Expr::UNARY_OP: {
            gen_expr_to_reg(e->child, fi, o, cur_sp_bytes, "t0");
            if (e->op_char == '-') {
    ...
    // Evaluate left (first) into stack to preserve across potential calls,
    // evaluate right into t1 without pushing, then pop left into t0.
    gen_expr_stack(e->left, fi, o, cur_sp_bytes);
    gen_expr_to_reg(e->right, fi, o, cur_sp_bytes, "t1");
...
static void gen_stmt(Stmt* s, const FuncInfo& fi, ostream &o, vector<pair<string,string>> &loop_stack, int &cur_sp_bytes) {
    ...
    switch (s->type) {
        ...
        case Stmt::EMPTY:
            break;
        case Stmt::EXPR:
            gen_expr_to_reg(s->expr_stmt, fi, o, cur_sp_bytes, "t0");
            break;
        case Stmt::DECLARE: {
            gen_expr_to_reg(s->declare_init, fi, o, cur_sp_bytes, "t0");
            int off = fi.stmt_lhs_offset.at(s);
            emit(o, "sw t0, " + off_s0(off));
            break;
        }
        case Stmt::ASSIGN: {
            gen_expr_to_reg(s->assign_rhs, fi, o, cur_sp_bytes, "t0");
            int off = fi.stmt_lhs_offset.at(s);
            emit(o, "sw t0, " + off_s0(off));
            break;
        }
        case Stmt::IF: {
            ...
            string Lend = lab.next("Lend");
            gen_expr_to_reg(s->if_cond, fi, o, cur_sp_bytes, "t0");
        ...
        case Stmt::WHILE: {
            ...
            emit(o, Lbegin + ":");
            gen_expr_to_reg(s->while_cond, fi, o, cur_sp_bytes, "t0");
            emit(o, "beqz t0, " + Lend);
